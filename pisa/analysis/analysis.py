    ...
    * Minimizer Interface (via method `_minimizer_callable`)
    ...
    def fit_hypo_inner(self, data_dist, hypo_maker, metric, minimizer_settings,
        ...
        fit_info : OrderedDict with details of the fit with keys 'metric',
            'metric_val', 'params', 'hypo_expected_dist', and
            'minimizer_metadata'
        ...
        hypo_expected_dist = hypo_maker.get_outputs(return_sum=True)
        ...
        if isinstance(hypo_maker, Detectors):
            fit_info['detailed_metric_info'] = [self.get_detailed_metric_info(
                data_dist=data_dist[i], hypo_expected_dist=hypo_expected_dist[i],
            ...
            fit_info['detailed_metric_info'] = self.get_detailed_metric_info(
                data_dist=data_dist, hypo_expected_dist=hypo_expected_dist,
        ...
        # If blind replace hypo_expected_dist with none object
        if blind:
            hypo_expected_dist = None
        fit_info['hypo_expected_dist'] = hypo_expected_dist
    ...
    def nofit_hypo(self, data_dist, hypo_maker, hypo_param_selections,
                   hypo_expected_dist, metric, other_metrics=None, blind=False, external_priors_penalty=None):
        ...
        hypo_param_selections : None, string, or sequence of strings
        hypo_expected_dist : MapSet or List of MapSets
        metric : string or iterable of strings
        other_metrics : None, string, or sequence of strings
        blind : bool
        external_priors_penalty : func
        ...
        # Assess the fit: whether the data came from the hypo_expected_dist
        try:
            if isinstance(hypo_maker, Detectors):
                metric_val = 0
                for i in range(len(hypo_maker._distribution_makers)):
                    data = data_dist[i].metric_total(expected_values=hypo_expected_dist[i],metric=metric[i])
                    metric_val += data
                priors = hypo_maker.params.priors_penalty(metric=metric[0]) # uses just the "first" metric for prior
                metric_val += priors
            else: # DistributionMaker object
                metric_val = (
                    data_dist.metric_total(expected_values=hypo_expected_dist,
                                           metric=metric[0])
    ...
    @staticmethod
    def get_detailed_metric_info(data_dist, hypo_expected_dist, params, metric,
        ...
        for m in all_metrics:
            ...
            name_vals_d['maps'] = data_dist.metric_per_map(
                expected_values=hypo_expected_dist, metric=m
            )
            metric_hists = data_dist.metric_per_map(
                expected_values=hypo_expected_dist, metric='binned_'+m
            )
            maps_binned = []
            for asimov_map, metric_hist in zip(hypo_expected_dist, metric_hists):
                map_binned = Map(
    ...
    def log_prob(self, scaled_param_vals, hypo_maker, data_dist, metric, counter,  
        ...
        hypo_maker : Detectors or DistributionMaker
            Creates the per-bin expectation values per map based on its param 
            values. Free params in the `hypo_maker` are modified by the sampler
        ...
        try:
            ...
            # if ValueError we're outside of allowed parameter range
            return -np.inf

        # Get the expectation values map set
        try:
            hypo_expected_dist = hypo_maker.get_outputs(return_sum=True)
        except Exception as e:
            if blind:
                ...
                logging.error(
                    'Failed to generate expected values with free'
        ...
        try:
            if isinstance(hypo_maker, Detectors):
                metric_val = 0
                for i in range(len(hypo_maker._distribution_makers)):
                    data = data_dist[i].metric_total(expected_values=hypo_expected_dist[i],
                                                  metric=metric[i])
                    metric_val += data
                priors = hypo_maker.params.priors_penalty(metric=metric[0]) # uses just the "first" metric for prior
                metric_val += priors
    ...
    def _minimizer_callable(self, scaled_param_vals, hypo_maker, data_dist,
        ...
        hypo_maker : Detectors or DistributionMaker
            Creates the per-bin expectation values per map based on its param 
            values. Free params in the `hypo_maker` are modified by the minimizer 
            to approximate their MLEs
    ...
    def scan(self, data_dist, hypo_maker, metric, hypo_param_selections=None,
        ...
        for i, pos in enumerate(loopfunc(*steplist)):
            ...
            if isinstance(best_fit['hypo_expected_dist'], Sequence):
                best_fit['hypo_expected_dist'] = [deepcopy(
                    best_fit['hypo_expected_dist'][i].serializable_state
                ) for i in range(len(best_fit['hypo_expected_dist']))]
            else:
                best_fit['hypo_expected_dist'] = deepcopy(
                    best_fit['hypo_expected_dist'].serializable_state
                )
            ...
            if debug_mode == 0 or debug_mode == 1:
                try:
                    ...
                    del best_fit['hypo_expected_dist']
                except KeyError:
                    pass
